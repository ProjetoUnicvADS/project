...
public class BaseEntityController {
    ...
    public ResponseEntity<Page<BaseEntityDto>> list(Pageable pageable) {
        List<BaseEntityDto> listEntitiesDto = baseEntityService.list(pageable).stream()
                .map(entity -> modelMapper.map(entity, BaseEntityDto.class)).toList();

        Page<BaseEntityDto> page = new PageImpl<>(listEntitiesDto);
